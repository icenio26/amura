...
| Hosting | VPS (IONOS / OVH / Hetzner) | **Cloudflare Pages** (gratis) | HTTPS, CDN global, despliegue automático en cada `git push`. Un VPS son 5-15 €/mes *más* ser tu propio sysadmin: actualizaciones, backups, fail2ban, certificados. |
...
El sitio de este repositorio es estático: se publica tal cual, sin build.

**Cloudflare Pages (recomendado)**
1. Cloudflare Dashboard → Workers & Pages → Create → Pages → Connect to Git.
2. Seleccionar este repositorio.
3. Build command: *(vacío)*. Build output directory: `/`.
4. Añadir el dominio propio en Custom domains.
